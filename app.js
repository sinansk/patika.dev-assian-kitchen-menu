...
/////Button Elements In DOM/////
let buttonDOM  = document.querySelector(".btn-container");
let menuDOM = document.querySelector(".section-center");

//////Menus///
const koreaFoods = menu.filter(x => x.category == "Korea");
const japanFoods = menu.filter(x => x.category == "Japan");
const chinaFoods = menu.filter(x => x.category == "China");

///////Create All Menu From Array With Function////
const createButton = (buttonName) => {
  let btn = `<button type="button" id="${buttonName}" class="btn btn-outline-danger btn-item">${buttonName}</button>`;
  return btn;
  };
//////Show Buttons In UI////
...
//////Create Single Food Item For Menu////
const createFood = (name) => {
  let item = `<div class="menu-items col-6">
                <img src="${name.img}" alt="${name.img}" class="photo">
               <div class="menu-info">
...
///Pass Menu Items Into createFood() function and render on screen////
const showAllFoods = () => {
  menuDOM.innerHTML = "";
  menu.forEach(name => {
